...
```{r, fig.fullwidth = TRUE, fig.height=3, fig.cap = paste0("Répartition des cas de diarrhées aigues par tranche d'age, ", list_dat[[list_depts]]$date %>% min %>% format('%d %b %Y'), " - ", list_dat[[list_depts]]$date %>% max %>% format('%d %b %Y'), " par commune, Grand’Anse (données partielles).")}
