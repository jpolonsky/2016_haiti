...
\footnotesize
```{r, tab_overview}
# print table if output is PDF document
...
if(knitr::opts_knit$get("rmarkdown.pandoc.to") %in% c('docx', 'html')) knitr::kable(list_table[[list_depts]])
```
...
```{r, fig.fullwidth = F, fig.height=3, fig.cap = paste0("Répartition des cas de diarrhées aigues par tranche d'age, ", list_dat[[list_depts]]$date %>% min %>% format('%d %b %Y'), " - ", list_dat[[list_depts]]$date %>% max %>% format('%d %b %Y'), " par commune, Grand’Anse (données partielles).")}
...
list_hist_week_inst[[list_depts]]
```
