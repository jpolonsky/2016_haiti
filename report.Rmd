...
# by week & commune
list_hist_post_hurr_week_commune <- map(list_dat_post_hurr, PlotHistWeekCommunePostHurrCTC) %>% set_names(list_depts)

# by week & inst
list_hist_post_hurr_week_CTC <- map(list_dat_post_hurr, PlotHistWeekInstPostHurrCTC) %>% set_names(list_depts)

# tables
list_table_post_hurr_CTC <- map(list_dat_post_hurr, MakeTablePostHurrCTC) %>% set_names(list_depts)
```
...
```{r, fig.fullwidth = TRUE, fig.height=4, fig.cap = paste0("Tendance des cas de diarrhées aigues, ", list_dat_post_hurr[[list_depts]]$date %>% min %>% format('%d %b %Y'), " - ", list_dat_post_hurr[[list_depts]]$date %>% max %>% format('%d %b %Y'), " par commune, Grand’Anse (données partielles).")}
list_hist_post_hurr_week_commune[[list_depts]]
```

\newpage

```{r, fig.fullwidth = TRUE, fig.height=5.5, fig.cap = paste0("Tendance des cas de diarrhées aigues, ", list_dat_post_hurr[[list_depts]]$date %>% min %>% format('%d %b %Y'), " - ", list_dat_post_hurr[[list_depts]]$date %>% max %>% format('%d %b %Y'), " par CTC, Grand’Anse (données partielles).")}
list_hist_post_hurr_week_CTC[[list_depts]]
```
